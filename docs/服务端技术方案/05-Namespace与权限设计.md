...
版本：设计 v1.2 · 2026-09-23 · 原型基线 v23。**设计规范，尚未实现鉴权、数据库、管理界面或 SDK 服务身份。** 本文是权限规则的唯一详述；表结构见 [数据库](./01-数据库设计.md)，请求协议见 [接口](./02-接口协议.md)，运行凭证见 [SDK](./03-SDK与分流协议.md)。

## 1. 边界与概念

```text
单企业 / 单部署                         无 tenant
├─ 全局 principal（人 / 机器）、用户组、角色目录
...
| 绑定 | 给某个 principal 或用户组，在某个资源及其后代授予角色；可附加环境与到期限制 |
| 用户组 | 全局人类用户集合，无嵌套；有效主体为组成员与有效 Namespace 成员的交集 |
...
flowchart LR
    ...
    RP --> A[permission / 稳定 action]
    B --> S[auth_resource / 真实归属范围]
    S --> C[后代资源]
```
...
`auth_resource` 有真实 parent 和到实体的强约束映射，不能把任意 `resource_type + resource_id` 当合法 scope。数据库复合 FK、固定实体类型、反向延迟约束和父链检查见 [数据库 §2](./01-数据库设计.md)。
...
- 实体归属、auth_resource.parent、拓扑版本、policy_revision 与审计同事务更新；清除相关缓存。
...
| 部署 IAM | `iam.roles.manage`、`iam.groups.manage` | deployment：维护角色模板、全局组及组成员；都可能扩大权限，必须审计 |
...
**每一列都是 AND。** 继承、组或多个绑定可以合并满足同一动作，但不能用某一个资源的授权代替整行检查。依赖集合由服务端按完整当前配置计算，包含所有祖先受众，不采信客户端声明的子集。
...
| 分析/查看/导出 | 分别 `analysis.execute` / `result.read` / `result.read`+`result.export` | 沿实验身份 | 展示的参数详情另需 read | 展示的指标定义需 read；分析任务按已冻结计划，不修改计划 |
...
```text
选择 Namespace → 选择成员或用户组 → 选择可委派角色
→ 选择可管理资源 scope → 可选环境 / 到期时间
→ 预览新增动作及覆盖范围 → 保存 → 审计 + policy_revision + 缓存失效
```
...
3. 直接主体是有效 Namespace 成员；组主体至少含一个有效成员，绑定仅对该交集生效。部署绑定按全局有效人类成员判断。
4. 被授动作不超过操作者在该范围可委派的动作。不能只凭 bindings.manage 发出任意角色。
5. 在事务锁内重检以上条件；变更、版本、审计和失效事件同事务提交。

Namespace 管理员可以绑定平台允许委派的角色，不能修改全局角色目录。新增用户组成员可能让多个 Namespace 的现有绑定生效：平台组管理员必须看到受影响范围，并记录审计；仍不能绕过个人成员资格。

### 5.2 撤回流程

```text
定位有效权限来源 → 撤销绑定 / 移出组 / 禁用成员 / 禁用身份
→ 重算其他 allow 来源 → 写审计并递增涉及 Namespace 的 policy_revision
→ 阻止新的管理请求与凭证请求
```

- 若父级或其他组仍授予相同动作，移除一条绑定后权限仍存在；界面明确显示剩余来源。
- 撤权不暗中停止正在运行的实验。业务需停止时，具备权限者另执行 `experiment.stop`。
- 到期即不生效；缓存有效期不能超过绑定、成员、凭证的最近到期时间。
...
| 审计页 | 操作者、主体、角色、scope、环境、变更前后、原因和请求 ID；可追溯组变更 |

按钮可见性只辅助操作；后端每次检查。列表按资源权限过滤，不能先全量返回再由浏览器隐藏。

## 6. SDK 与服务身份

机器凭证绑定唯一 `Namespace + environment + service + principal`，动作由凭证上限与当前角色绑定取交集；machine 也要有效成员资格。控制台角色不能自动转换成 SDK 凭证。
...
| 配置下载/manifest 续发 | runtime.config.read；projection/capability/service 匹配，当前策略版本及撤销状态 |
...
撤销凭证立即阻止新鉴权请求；已发短租约与上下文按既有 TTL/撤销栅栏处理，不能声称瞬间让所有离线执行消失。人员授权缓存与运行配置缓存分开；停止运行及桶回收仍遵循 [SDK 协议](./03-SDK与分流协议.md) 和数据库的 draining 规则。
...
权限缓存键至少包含 `principal + namespace + action + resource + environment + policy_revision`；有效期还受最近到期时间约束。Namespace 的 policy_revision 单调递增，不随回滚降低。

| 变化 | 失效范围 |
| --- | --- |
| 成员/绑定/凭证/资源父关系 | 当前 Namespace |
| 全局组成员或组状态 | 所有引用该组的 Namespace |
| principal 禁用 | 该主体关联的所有 Namespace |
| 角色动作或状态 | 所有引用该角色的 Namespace |

写请求在锁内检查当前策略；发布激活必须重检发布者和全部依赖。长时间审批/编译不冻结权限。审计与变更同事务，通过 outbox 通知缓存；收到失效事件前也不能用旧版本缓存批准写入。

## 8. 分阶段实施

| 阶段 | 交付 | 验收重点 |
| --- | --- | --- |
| 1. 作用域与目录 | Namespace 更名迁移、显式成员、auth_resource 强约束、稳定动作与平台角色模板 | ID/盐保留；跨 Namespace FK/引用拒绝；默认拒绝 |
| 2. 管理面授权 | principal/group 绑定、环境限定、继承、列表过滤、多资源检查、审计/缓存 | 父级 allow 无法被子级只读撤销；组不绕过成员；审批不要求编辑；隐藏信息不泄漏 |
| 3. 服务身份 | 机器成员、受限凭证、运行 API 校验、短租约/撤销 | 任意参数来源均不越应用范围；撤销阻止新请求；离线边界明确 |
| 4. 并发与回放 | 全局变更失效、撤权竞态、拓扑迁移授权、旧 grant 转换报告 | 撤权与发布竞态；组加人扩大权限可审计；恢复不降低策略/撤销版本 |

当前仅更新设计文档；已有前端原型的名称、按钮显隐或 mock 数据不构成后端权限实现。未建表、未迁移旧授权、未签发生产凭证，也未执行上述服务端验收。
