...
| 概念 | 语义 | 可否在运行中原地改变 |
| --- | --- | --- |
| `tenant_id / project_id / environment_id` | 租户、业务实验空间、开发／测试／生产环境 | 不可由请求正文切换授权范围 |
...
所有查询、缓存 Key、对象路径、事件路由、异步任务携带并核验租户／项目／环境。鉴权由可信会话或凭据确定租户，正文中的 ID 只是待验证引用。数据库使用复合外键与应用层权限检查；日志和错误不暴露其他租户资源是否存在。
