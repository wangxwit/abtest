...
```mermaid
flowchart TB
    U[全局账号 / 用户组] --> M[Namespace 成员与资源授权]
    M --> N[Namespace]
    N --> C[应用 / 参数 / 私有标签]
    N --> A[画像属性 / 受众 / 指标目录]
    N --> E[开发 / 测试 / 生产环境]
...
| `policy_revision` | Namespace 授权版本 | 授权变更递增并失效相关缓存 |
...
拒绝审核将 run 结束为 `review_rejected`，另建可编辑草稿；旧提交映射保留。只有确认从未激活、未签发运行 token 的预留可直接释放，其余进入 draining。

### 提交事务

1. 校验当前成员、资源权限、版本及依赖。
2. 按固定顺序锁授权版本、环境拓扑和目标层账本。
3. 严格解析配置，重检受众证明与可用桶。
4. 写 revision、run、分组快照和预留，标记来源草稿已提交。
...
产物携带 revision、输入摘要、编译器／Schema／hash 版本、摘要与时间。签名 manifest 绑定内容、授权范围和有效期。允许对相同内容签发新租约，但不能覆盖旧 manifest 或绕过撤销。摘要规范采用 [RFC 8785 JCS](https://www.rfc-editor.org/rfc/rfc8785)。
...
| 操作 | 约束 |
| --- | --- |
| 回滚 | 新建发布，按当前撤销集和桶账本校验；不能复活已回收桶，也不删除事件 |
...
- 新建或撤销授权同步审计并失效缓存；撤销人的权限不自动停止已运行实验。
...
| P1 管理 | 成员授权、目录、拓扑、草稿、预留、审批 | 无越权、无双占、无丢稿；跨空间／环境不串数据 |
...
必须覆盖：并发争抢最后流量、幂等键正文冲突、授权撤回与提交竞态、跨 Namespace／环境引用、组授权越权、参数激活并发拆层、固定资格桶复用、暂停恢复不换组、半包失败、撤销与最长停用时限、Unicode hash、多服务回退保留分配、无事件用户分母、迟到重算。
