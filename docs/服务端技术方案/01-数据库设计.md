...
版本：设计 v1.0 · 2026-09-19 · 原型基线 v23。本文是 PostgreSQL 16+ 的逻辑模型和事务设计，**尚未建库、执行迁移或实现后端**。SQL 仅说明核心约束，不能作为全量迁移执行。
...
| 记号 | 展开的字段／约束 |
| --- | --- |
| `T` | `tenant_id uuid`；除tenant本表外 FK → tenant |
| `P` | `tenant_id uuid, project_id uuid`；复合 FK → `project(T,project_id)` |
| `E` | `tenant_id uuid, project_id uuid, environment_id uuid`；复合 FK → `environment(P,environment_id)` |
| `C` | `created_at timestamptz, created_by uuid`；操作者 FK → `principal(T,principal_id)` |
...
| `G` | `owner_id uuid, lifetime text CHECK IN ('long_term','temporary'), expected_end_date date?`；owner FK → `principal(T,principal_id)`；长期日期为空、临时日期必填 |
...
表中 `PK(E,id)` 表示实际复合主键含 E 三列；索引、唯一约束和 FK 同理。即使 UUID 全局唯一，也不只依赖单列 ID：环境内 FK 必须携带 E，项目目录引用携带 P，账号引用携带 T。禁止用无约束的 `resource_type + resource_id` 保存核心业务关系。除专门说明，外键使用 `ON DELETE RESTRICT`，不级联删除历史。

所有 FK 的引用端建立以完整 FK 字段为前缀的 B-tree 索引；若已有 PK／UQ／查询索引覆盖前缀则不重复建。PostgreSQL 不自动为引用端创建索引；复合 FK 用来防止跨租户、跨项目、跨环境错误引用。[PostgreSQL：约束与外键](https://www.postgresql.org/docs/16/ddl-constraints.html)

公共索引约定：带 `A` 的目录对象建立 `(scope, archived_at, created_at DESC, id)` 列表索引；带 `owner_id` 的对象另建 `(scope, owner_id, archived_at)`。JSON 不默认全表 GIN，只有实际检索需求和执行计划证明必要时才增加。
...
| --- | --- | --- |
| `tenant` | `T, name text, status text, created_at timestamptz` | PK(T)；status=`active/suspended` |
| `principal` | `T, principal_id uuid, identity_issuer text, identity_subject text, kind text, display_name text, status text, created_at timestamptz` | PK(T,principal_id)；UQ(T,identity_issuer,identity_subject)；kind=`human/machine`；status=`active/disabled` |
| `project` | `T, project_id uuid, project_key text, name text, timezone text, M, A` | PK(T,project_id)；UQ(T,project_key)，归档不释放 Key |
...
ALTER TABLE parameter ADD CONSTRAINT parameter_project_key_uq
  UNIQUE (tenant_id, project_id, parameter_key);

-- 同域同参数恰一层：PK防重复；“没有漏项”仍由提交末尾差集校验。
ALTER TABLE layer_parameter ADD CONSTRAINT layer_parameter_pk
  PRIMARY KEY (tenant_id, project_id, environment_id, domain_id, parameter_id);
ALTER TABLE layer_parameter ADD CONSTRAINT layer_same_domain_fk
  FOREIGN KEY (tenant_id, project_id, environment_id, domain_id, layer_id)
  REFERENCES layer (tenant_id, project_id, environment_id, domain_id, layer_id);
...
CREATE UNIQUE INDEX one_control_per_revision ON experiment_variant
  (tenant_id, project_id, environment_id, experiment_id, experiment_revision)
  WHERE role = 'control';
CREATE UNIQUE INDEX one_open_run_per_experiment ON experiment_run
  (tenant_id, project_id, environment_id, experiment_id)
...
数据库运行账号不拥有表、无超级用户或BYPASSRLS。租户表启用并强制RLS，项目／环境策略按认证中间件验证后的事务本地上下文 `SET LOCAL` 读取；连接池事务结束即清理上下文。RLS是纵深防御，应用仍需校验人类与机器主体的资源动作权限。表owner和具有BYPASSRLS的角色可能绕过策略，不能把默认RLS当管理员隔离保证。[PostgreSQL：行级安全](https://www.postgresql.org/docs/16/ddl-rowsecurity.html)

项目级事件使用独立的 `project_audit_event(P, audit_id, actor_id, action, object_reference, request_id, before_digest?, after_digest?, reason, occurred_at, metadata)`，字段类型与audit_event相同、PK(P,audit_id)、actor FK按T、时间/操作者索引同构；不能为无环境的注册操作随便填一个生产environment_id。项目级outbox也使用独立 `project_outbox_event`（将outbox_event的E替换为P，其余字段与索引不变）。
