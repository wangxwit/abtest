...
版本：设计 v1.2 · 2026-09-23。本文是服务端待实施方案；当前前端原型的已有图表是演示数据，没有真实采集、仓库或统计引擎。统一对象见[方案总览](./README.md)，采集接口见[接口协议](./02-接口协议.md)，决策与曝光时机见[SDK 与分流协议](./03-SDK与分流协议.md)。
...
| 对象 | 不变量 |
| --- | --- |
| 目录 | 指标、参数、受众定义及版本属于 Namespace；引用不得跨 Namespace |
...
服务端注入 scope，校验真实资源归属；查询、游标、缓存和导出绑定 principal／授权范围及 policy_revision。撤权阻止新请求并使旧 allow 缓存失效；不自动停止实验或删除历史事实。UI 隐藏不可读对象，后台仍按完整冻结计划计算，不因读者权限裁剪 cohort 或更改统计分母。

analysis_plan 在启动前冻结，引用不可变指标版本；展示名称／备注可改。正式口径修复须审计、失效原结果并重新评审，不得事后择优。
...
| 任务幂等 | 固定计划、快照、cutoff、代码、seed；规范摘要作幂等键，临时写后唯一发布 |
| 修复／重算 | 新 analysis_run_id + supersedes_analysis_run_id、差异和原因；旧批次只读，错误 final 追加失效／替代记录 |

result_analysis_run 保存 scope、run、analysis_plan_version_id、metric_version_ids、code_version、source_snapshot_ids、源 offsets、event_time_watermark、received_at_cutoff、attribution_cutoff、correction_horizon、dataset_version、seed、时间、发起者、质量报告和 URI。重算覆盖真实迟到／更正范围，不固定“只算昨天”。复现期内保持快照可读；过期／依法删除后标明无法完整复现。
