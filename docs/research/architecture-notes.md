...
| 一期控制库 30 张表 | 独立身份和关系保留表；完整配置随运行冻结，通用基础设施合并，避免重复维护授权树；不是论文要求 |
...
- 独立查询、授权或生命周期的核心对象保留关系表与作用域约束。
- 一起读取、一起冻结的分组配置和分析计划保存为有版本的快照；JSON 仍须严格校验。
- 权限从业务归属关系计算，不维护第二棵可变资源树；先直查当前权限，再按实际负载考虑缓存。
- 审计、Outbox、幂等各用一张表，作用域与唯一键仍完整约束。
- 配置文件、应用投影和参数包存对象仓库；不能把其他应用的内容打包下发给普通服务。
- 事件与湖仓属于已有数据链路，不把控制库表数当成整个平台存储总数。

具体 30 表及原模型去向见[数据库设计](../服务端技术方案/01-数据库设计.md)。这些是本平台的一期取舍，尚未通过生产运行验证。
