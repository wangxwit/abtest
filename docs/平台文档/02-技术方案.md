...
| 演示团队、负责人字段 | 全局个人／机器身份、多对多成员、直接资源角色绑定与后台鉴权 |
| 本地 plan／validate | 环境内核心管理事务串行、版本检查、统一审计和 Outbox |
| 演示审核与状态按钮 | experiment_run 冻结配置、统一 change_request 审批、发布协调与撤销栅栏 |
...
一期生产设计为 **30 张物理控制表**，不含现有事件湖仓事实表；它不是整条数据平台的总表数。具体清单见[数据库设计](../服务端技术方案/01-数据库设计.md)。

| 简化点 | 保留的契约 |
| --- | --- |
| `role` 保存动作集合，由代码动作目录校验 | 直接个人／机器角色绑定、Namespace 成员和资源范围不缩水 |
| 沿真实业务归属判定权限 | 不复制 `auth_resource` 树；引用边仍不继承 |
| 环境内核心管理事务串行 | 保留版本、依赖和完整权限校验，简化细粒度锁 |
| `experiment_run` 合并原一对一 revision、冻结分组值和分析计划 | 改变处理含义仍新建 run，不能改写旧运行 |
| `metric` 保存当前定义，run 冻结实际分析口径 | 更新目录不改变旧 run 的指标口径 |
| `change_request` 统一审批；审计、Outbox、幂等各一张带作用域的表 | 仍记录目标、版本、审批与操作结果 |
| 快照、参数包、服务投影存对象存储 | 不逐类建关系表；保留摘要、签名、租约、撤销与执行追踪 |

用户组、组绑定、权限缓存及通用策略注册框架后续扩展，一期不提供对应 API／表。SDK 的运行配置缓存、短租约、撤销栅栏和桶 draining 继续保留。

## 4. Namespace 与授权接入

```text
个人／机器身份 → 验证 namespace_member 有效（仅准入）
→ 读取当前成员、角色及直接资源绑定；一期不使用权限缓存
...
权限沿真实业务归属继承：应用→参数、环境→域→层→实验。引用边不能继承：能用层不意味着能修改其参数。默认拒绝、allow 并集、无显式 deny；子级只读不能覆盖父级已授编辑。平台管理全局角色模板，Namespace 管理员仅可委派获准角色和范围。
