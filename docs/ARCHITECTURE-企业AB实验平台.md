...
一期生产控制库恰为 **30 张物理表**，不含现有事件湖仓事实表，也不是整条数据平台的总表数。环境内核心管理事务串行，简化锁而保留完整校验。

| 存储归并 | 一期设计 |
| --- | --- |
| 运行契约 | `experiment_run` 合并原一对一实验 revision、冻结分组值、分析计划及实际指标口径；`metric` 维护当前定义 |
| 授权 | 个人／机器直接绑定资源角色；动作集合存 `role`，由代码目录校验；沿真实业务归属判断，不复制授权资源树 |
| 管理记录 | `change_request` 统一审批；审计、Outbox、幂等各一张带作用域的表 |
| 运行产物 | 配置快照、参数包和服务投影存对象存储，不逐类建关系表；签名、租约、撤销与桶 draining 保留 |

全局用户组、组绑定、权限缓存和通用策略注册框架后续扩展，一期不提供对应 API／表。详细清单见[数据库设计](./服务端技术方案/01-数据库设计.md)，授权边界见[权限设计](./服务端技术方案/05-Namespace与权限设计.md)。

```mermaid
flowchart TB
  UI[管理控制台] --> API[控制 API／后台鉴权]
  IAM[个人／机器／直接角色绑定] --> API
  API --> DB[(30 张控制表)]
  DB --> Outbox[事务 Outbox]
  Outbox --> Compiler[完整校验／配置编译]
  Compiler --> Store[(对象存储：快照／参数包／服务投影)]
...
平台个人／机器身份与业务随机化 `user_id` 分离。身份可加入多个 Namespace，成员仅准入；一期直接绑定角色，按动作、资源范围和可选环境限制判定。目录属于 Namespace，环境下拓扑、预留、发布、凭证与事件独立。
...
- **继承**：仅沿真实业务归属继承，引用边不继承；环境授权不授目录全局编辑。多资源操作检查全部必要权利。
...
Namespace 管理员只能委派平台允许的角色和范围；机器 SDK 按 ns/env/service/action 授权。一期管理鉴权读取当前成员、角色与直接绑定，不做权限缓存；SDK 运行配置缓存继续受租约和撤销限制。完整判定和审计模型见[权限设计](./服务端技术方案/05-Namespace与权限设计.md)。
